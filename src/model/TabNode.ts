...
export class TabNode extends Node implements IDraggable {
    ...
    isFloating() {
        const floatings = this.model.getFloatingsMap();
        for (const floating of floatings.values()) {
            if (floating.tabId === this.getId()) {
                return true;
            }
        }
        return false;
    ...
    getFloatingId() {
        const floatings = this.model.getFloatingsMap();
        for (const [floatingId, floating] of floatings.entries()) {
            if (floating.tabId === this.getId()) {
                return floatingId;
            }
        }
    ...
    private static createAttributeDefinitions(): AttributeDefinitions {
        ...
        attributeDefinitions.addInherited("enablePopout", "tabEnablePopout").setType(Attribute.BOOLEAN).setDescription(
